...
console.log(`Testing RELATIVE calculateGeometryOffset\n${'='.repeat(70)}\n`);
...
// Load reference model (HUM_BODY_NAKED0)
const refPath = `/Users/artur/dev/gothic/ZenKit/public/game-assets/ANIMS/_COMPILED/HUM_BODY_NAKED0.MDM`;
const refBuffer = readFileSync(refPath);
const refArray = new Uint8Array(refBuffer);

const refMeshLoader = ZenKit.createModelMeshLoader();
const refLoadResult = refMeshLoader.loadFromArray(refArray);

if (!refLoadResult.success) {
    console.log('❌ Failed to load reference model');
    process.exit(1);
}

const refModel = ZenKit.createModel();
refModel.setMesh(refMeshLoader.getMesh());
const refMeshes = refModel.getSoftSkinMeshes();
const referenceMesh = refMeshes.get(0);

console.log(`✅ Loaded reference model: HUM_BODY_NAKED0\n`);
console.log(`${'='.repeat(70)}\n`);

const testModels = [
    { name: 'HUM_BODY_NAKED0', expected: '~0 (same as reference)' },
...
for (const test of testModels) {
    try {
        ...
        // Calculate RELATIVE offset (to reference)
        const ssm = softSkinMeshes.get(0);
        const offset = model.calculateGeometryOffset(ssm, hierarchy, referenceMesh);
        
        console.log(`📦 ${test.name}:`);
        console.log(`   Expected: ${test.expected}`);
        console.log(`   Relative offset: (${offset.x.toFixed(3)}, ${offset.y.toFixed(3)}, ${offset.z.toFixed(3)})`);
        console.log(`   ✅ Apply: geometry.translate(${offset.x.toFixed(2)}, ${offset.y.toFixed(2)}, ${offset.z.toFixed(2)})`);
        console.log();
...
console.log('\n💡 EXPLANATION:');
console.log('   Offset = (current_model_ideal_position) - (reference_model_ideal_position)');
console.log('   This ensures all models align to the same skeleton coordinate system!');
